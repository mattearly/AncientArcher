...
void FirstPersonPlayer::update(float deltaTime)
{
  ...
  // while the Y is unlocked or you are jumping
    if (moves.back || moves.forward) {
      ...
      if (moves.forward)     model.get()->getEntityPtr()->moveBy(moveFront * velocity);
      else if (moves.back)   model.get()->getEntityPtr()->moveBy(-moveFront * velocity);
    }
    if (moves.right) model.get()->getEntityPtr()->moveBy(*g_camera.getRight() * velocity);
    else if (moves.left) model.get()->getEntityPtr()->moveBy(-(*g_camera.getRight()) * velocity);
