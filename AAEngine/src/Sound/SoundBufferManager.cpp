	...
	p_PlayId = LOOP->addToSlowUpdate([]()
		{
			...
			while (processed > 0)
			{
				...
				alSourceUnqueueBuffers(p_SourceId, 1, &bufid);
				processed--;

				/* Read the next chunk of data, refill the buffer, and queue it
				 * back on the source */
				slen = sf_readf_short(p_LongSounds.at(p_PlayWhichId)->sndfile, p_LongSounds.at(p_PlayWhichId)->membuf, BUFFER_SAMPLES);
				if (slen > 0)
				{
					slen *= p_LongSounds.at(p_PlayWhichId)->sfinfo.channels * (sf_count_t)sizeof(short);
					alBufferData(bufid, p_LongSounds.at(p_PlayWhichId)->format, p_LongSounds.at(p_PlayWhichId)->membuf, (ALsizei)slen,
						p_LongSounds.at(p_PlayWhichId)->sfinfo.samplerate);
					alSourceQueueBuffers(p_SourceId, 1, &bufid);
				}
				if (alGetError() != AL_NO_ERROR)
				{
			...
			if (state != AL_PLAYING && state != AL_PAUSED)
			{
				...
				alGetSourcei(p_SourceId, AL_BUFFERS_QUEUED, &queued);
				if (queued == 0)
					return;
